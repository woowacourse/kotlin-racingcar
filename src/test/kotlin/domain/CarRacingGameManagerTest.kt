    ...
    inner class `생성할 때` {
        ...
        inner class `자동차 중 중복된 것이 있으면` {
            ...
            fun `예외 메세지를 포함한 IllegalArgumentException을 던진다`() {
                assertThatIllegalArgumentException().isThrownBy { CarRacingGameManager(duplicatedCars) }
        ...
        inner class `자동차의 개수가 2 이상 20 이하가 아니라면` {
            ...
            fun `예외 메세지를 포함한 IllegalArgumentException을 던진다`(cars: List<Car>) {
                assertThatIllegalArgumentException().isThrownBy { CarRacingGameManager(cars) }
                    .withMessageContaining(ERROR_MESSAGE)
            }
        ...
        inner class `중복되지 않은 2대 이상 20대 이하의 자동차가 주어지면` {
            ...
            fun `예외를 던지지 않는다`() {
                ...
                assertThatCode { CarRacingGameManager(cars) }.doesNotThrowAnyException()
            }
        }
    }
