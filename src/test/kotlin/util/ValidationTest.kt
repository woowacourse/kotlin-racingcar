...
class ValidationTest {
    ...
    fun `유효한 자동차 이름 입력 테스트`(carsName: String) {
        assertDoesNotThrow { InputView.Validation.carsName(carsName) }
    }
    ...
    fun `유효하지 않은 자동차 이름 입력 테스트`(carsName: String) {
        assertThrows<IllegalArgumentException> {
            InputView.Validation.carsName(carsName)
        }
    }
    ...
    fun `유효한 시도횟수 입력 테스트`(attemptCount: String) {
        assertDoesNotThrow { InputView.Validation.attemptCount(attemptCount) }
    }
    ...
    fun `유효하지 않은 시도횟수 입력 테스트`(attemptCount: String) {
        assertThrows<IllegalArgumentException> {
            InputView.Validation.attemptCount(attemptCount)
        }
    }
}
