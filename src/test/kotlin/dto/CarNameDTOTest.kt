...
class CarNameDTOTest {
    ...
    fun `자동차 이름에 오류가 없으면 에러가 발생하지 않는다`(input: String) {
        assertDoesNotThrow { CarsDTO(input) }
    }

    @ParameterizedTest
    @ValueSource(strings = ["", "pobi", "a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u"])
    fun `자동차 갯수가 2대 이상 20대 이하가 아니면 에러가 발생한다`(input: String) {
        val exception = assertThrows<IllegalArgumentException> { CarsDTO(input) }
        assertThat(exception.message).isEqualTo(CarsDTO.ERROR_CAR_COUNT)
    }
    ...
    fun `자동차 이름에 공백이 있거나 영문 소문자가 아니면 에러가 발생한다`(input: String) {
        val exception = assertThrows<IllegalArgumentException> { CarsDTO(input) }
        assertThat(exception.message).isEqualTo(CarsDTO.ERROR_NAME)
    }
    ...
    fun `자동차 이름이 1글자 이상 5글자 이하가 아니면 에러가 발생한다`(input: String) {
        val exception = assertThrows<IllegalArgumentException> { CarsDTO(input) }
        assertThat(exception.message).isEqualTo(CarsDTO.ERROR_NAME_LENGTH)
    }
    ...
    fun `자동차 이름에 중복이 있으면 에러가 발생한다`(input: String) {
        val exception = assertThrows<IllegalArgumentException> { CarsDTO(input) }
        assertThat(exception.message).isEqualTo(CarsDTO.ERROR_CAR_UNIQUENESS)
    }
}
