...
object Constants {
    ...
    const val INVALID_NAME_SIZE_EXCEPTION =
        "이름은 $MINIMUM_NAME_LENGTH~${MAXIMUM_NAME_LENGTH}글자 사이로 입력해주세요"
    const val DUPLICATED_NAME_EXCEPTION = "서로 중복되지 않는 이름을 입력해주세요."
    const val INVALID_NUM_OF_CARS_EXCEPTION = "${MINIMUM_CARS_NUMBER}대 이상의 자동차가 필요합니다."
    const val INVALID_TRIAL_NUM_TYPE_EXCEPTION = "숫자를 입력해주세요."
    const val INVALID_TRIAL_NUM_RANGE_EXCEPTION = "${MINIMUM_TRIAL_NUM}보다 큰 수를 입력해주세요."
